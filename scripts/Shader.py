...
class Shader:
    ...
    def getShaderJson(self):
        shader_path = None
        try:
            shader_path = self.ownerComp.par.Path.eval()
            with open(shader_path) as shader_file:
                json_shader = json.load(shader_file)
                print('json_shader', json_shader)
                self.stored['Shader'] = json_shader
                print('json stored!')

        except Exception as e:
            print(shader_path, 'not found. error:', e)
            debug(e)
            return False
        return True
    ...
    def loadSamplers(self, comp, samplers):
        ...
        for i in range(len(samplers)):
            ...
            sComp.op('audiofilein1').bypass = not (ctype == 'music')
